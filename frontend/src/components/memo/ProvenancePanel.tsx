...
type IntegrityState = "pending" | "verified" | "tamper" | "unknown" | "fallback";
...
function computeState(provenance: MemoProvenance): IntegrityState {
  ...
  // Fallback memos anchor a real keccak256 over a body that was NOT
  // generated by ChainGPT. Surfacing this as plain "Verified" would
  // misrepresent the integrity story — the body matches the hash but
  // the source is the local fallback, not the LLM. Render a distinct
  // amber state so the Provenance panel is honest about both facts.
  if (provenance.isFallback) {
    return provenance.verified ? "fallback" : "tamper";
  }
...
const BADGE = {
  ...
  fallback: {
    icon: FileWarning,
    title: "Fallback memo anchored",
    body: "The on-chain hash matches a memo body composed locally because ChainGPT was unavailable at generation time. The audit trail is still intact (keccak matches what is anchored on chain) but the body source is the local fallback path, not the LLM.",
    container: "bg-warning/10 border-warning/40",
    iconClass: "text-warning",
    titleClass: "text-warning",
    bodyClass: "text-warning/80",
